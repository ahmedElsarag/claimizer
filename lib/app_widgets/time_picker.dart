...
class _TimePickerInputState extends State<_TimePickerInput> with RestorationMixin {
  ...
  void initState() {
    super.initState();
    _selectedTime = RestorableTimeOfDay(widget.initialSelectedTime);

  }


   RestorableBool hourHasError = RestorableBool(false);
   RestorableBool minuteHasError = RestorableBool(false);
...
class _HourMinuteTextFieldState extends State<_HourMinuteTextField> with RestorationMixin {
  ...
  void didChangeDependencies() {
    super.didChangeDependencies();
    // Only set the text value if it has not been popu  d with a localized
    // version yet.
  ...
  Widget build(BuildContext context) {
    ...
    // If screen reader is in use, make the woosh text say hours/minutes.
    // Otherwise, remove the woosh text when focused because the centered cursor
    // appears odd above the woosh text.
    //
...
class _TimePickerDialogState extends State<TimePickerDialog> with RestorationMixin {
   GlobalKey<FormState> _formKey = GlobalKey<FormState>();
    ...
    RestorableTimeOfDay _selectedTime;
   _RestorableTimePickerMode _mode = _RestorableTimePickerMode(_TimePickerMode.hour);
   _RestorableTimePickerModeN _lastModeAnnounced = _RestorableTimePickerModeN(null);
   _RestorableAutovalidateMode _autovalidateMode = _RestorableAutovalidateMode(AutovalidateMode.disabled);
   RestorableBoolN _autofocusHour = RestorableBoolN(null);
   RestorableBoolN _autofocusMinute = RestorableBoolN(null);
   RestorableBool _announcedInitialTime = RestorableBool(false);

      VoidCallback _entryModeListener;
  ...
  void initState() {
    super.initState();
    _entryMode = _RestorableTimePickerEntryMode(widget.initialEntryMode);
    _selectedTime = RestorableTimeOfDay(widget.initialTime);
  ...
  Size _dialogSize(BuildContext context) {
    ...
    // to 1.1 as that provides enough space to reasonably fit all the content.
     double textScaleFactor = math.min(MediaQuery.of(context).textScaleFactor, 1.1);
...
Future< TimeOfDay> showCheckinTimePicker({
